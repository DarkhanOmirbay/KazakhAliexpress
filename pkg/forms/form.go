...
func (f *Form) MoreThanZero(field string, d int) {
	valueStr := f.Get(field)
	value, err := strconv.Atoi(valueStr)
	if err != nil {
		f.Errors.Add(field, "Error converting string to int")
		return
	}
	if valueStr == "" {
		return
	...
	if value <= 0 {
		f.Errors.Add(field, fmt.Sprintf("This field is less or equal to %d", d))
	}
